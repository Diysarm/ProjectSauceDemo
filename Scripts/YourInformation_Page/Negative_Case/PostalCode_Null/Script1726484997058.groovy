...
'Clear text yang ada pada text field FirstName'
WebUI.clearText(findTestObject('YourInformation_Page/TextArea_FirstName'))

'Clear text yang ada pada text field LastName'
WebUI.clearText(findTestObject('YourInformation_Page/TextArea_LastName'))

'Clear text yang ada pada text field PostalCode'
WebUI.clearText(findTestObject('YourInformation_Page/TextArea_PostalCode'))

'refresh page karena terkadang ada bug data tetap tersimpan walaupun sudah diclear'
WebUI.refresh()
...
WebUI.verifyElementVisible(findTestObject('YourInformation_Page/Label_ErrorMessage'))

'Mengambil teks error message pada field error message dan store ke variabel'
String errorMessagePostalCodeNull = WebUI.getText(findTestObject('YourInformation_Page/Label_ErrorMessage'))

'Jika error message sudah sesuai akan menamppilkan error message sudah sesuai jika belum akan menampilkan BELUM sesuai'
if (errorMessagePostalCodeNull.equals(errorPostalCodeNull)) {
    println('Error message sudah sesuai: ' + errorMessagePostalCodeNull)
} else {
   KeywordUtil.markFailed('Error message BELUM sesuai: ' + errorMessagePostalCodeNull)
}
